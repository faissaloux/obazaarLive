        ...
        <div class="row g-3">
            ...
            <div class="card weekly-product-card">
              <div class="card-body d-flex align-items-center">
                <div class="product-description"><p class="product-title d-block" >{{ $addresse->given_name }}</p>
                    ...
                    <div class="row">
                        <a class="col-md-6 adresses-btn btn btn-success btn-sm" href="{{ route('mobile.account.adresses.edit',['id' =>  $addresse->id ]) }}"></i>{{ __('Edit') }}</a>
                        <a class="col-md-6 adresses-btn btn btn-success btn-sm" href="{{ route('mobile.account.adresses.delete',['id' =>  $addresse->id ]) }}"></i>{{ __('Delete') }}</a>
                    </div>
                    
                    @if(!$addresse->is_primary) 
                        <div class="row">
                            <a class="col-md-12 adresses-btn btn btn-success btn-sm" href="{{ route('mobile.account.adresses.default',['id' =>  $addresse->id ]) }}">{{ __('set As Default') }}</a>
                        </div>
                    @endif
                </div>
              </div>
          ...
          <div class="col-12">
            <div class="select-all-products-btn">
                <a class="btn btn-danger w-100" href="{{ route('mobile.account.adresses.create', [  'store' => \Session::get('store')]) }}">{{ __('+ New Address') }}</a>
            </div>
          </div>
        </div>
      </div>
